...
impl Plugin for RobotDragPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<DragState>()
            .add_systems(Update, (handle_robot_drag_input, update_dragged_robot));
    }
}
...
pub fn handle_robot_drag_input(
    ...
    robot_query: Query<
        ...
        (With<DraggableRobot>, With<crate::RobotChassis>),
    >,
) {
        ...
        if let Some(cursor_position) = window.cursor_position() {
            ...
            if let Ok(ray) = camera.viewport_to_world(camera_transform, cursor_position) {
                ...
                for (robot_entity, robot_transform) in robot_query.iter() {
                    ...
                    let closest_point =
                        closest_point_on_ray_to_point(ray.origin, *ray.direction, robot_position);
...
pub fn update_dragged_robot(
    ...
    let Some(drag_entity) = drag_state.drag_entity else {
        return;
    };
    let Ok(window) = window_query.single() else {
        return;
    ...
    let Ok(mut robot_transform) = robot_query.get_mut(drag_entity) else {
        return;
    };
...
pub fn make_robot_draggable(
    ...
    robot_query: Query<Entity, (With<crate::RobotChassis>, Without<DraggableRobot>)>,
) {
