...
use crate::{
    ObliquePerspectiveProjection, ObliqueProjectionController, RobotChassis,
    camera::PanOrbitCamera, lidar::LidarSensor,
};
...
pub fn toggle_lidar_visualization(
    ...
    if keyboard.just_pressed(KeyCode::KeyL) {
        for mut lidar in lidar_query.iter_mut() {
            ...
            info!(
                "LIDAR visualization: {}",
                if lidar.visualize { "ON" } else { "OFF" }
            );
        }
    }
    ...
    if keyboard.just_pressed(KeyCode::KeyO) {
        for mut lidar in lidar_query.iter_mut() {
            ...
            info!(
                "LIDAR obstacle logging: {}",
                if lidar.enable_logging { "ON" } else { "OFF" }
            );
        }
    }
}
